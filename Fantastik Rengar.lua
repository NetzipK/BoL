...
function Combo()
	if ValidTarget(target) then
		...
		if Config.CSet.UseQ and GetDistance(target) <= 200 and QREADY then
			if Config.Misc.QSet.QReset and not R.Invisible then
				if isSX and not SxOrb:CanAttack() then
					CastSpell(_Q)
				elseif isSAC and not _G.AutoCarry.Orbwalker:CanShoot() then
					CastSpell(_Q)
				end
			elseif not Config.Misc.QSet.QReset and not R.Invisible then
				CastSpell(_Q)
			end
			if R.Invisible and GetDistance(target) <= AA.PassiveRage + 100 then
				CastSpell(_Q)
			end
		end
